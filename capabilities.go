...
func stateBrief(home string) string {
	...
	b.WriteString("- **Identify** — `export SELF_CALLER=\"<who you are>\"` before writing anything; it is recorded verbatim as `by` on every event you cause. Skip it and your events are indistinguishable from every other caller's: on a shared instance no reader can tell which agent wrote a line, and you cannot filter your own writes out of what you read back. `SELF_MIND_ID` does the same for the receipts of scripts you author.\n")
