...
Reading appends nothing, so orient until you actually know; what reading spends is context, not state. Views print to stdout, so filter them where you stand — `self view log | tail -30`. Filtering returns records, not current state: reconstructing which records still stand is the fold a view holds, and when a name in this log has no view, writing one is the durable work. You cannot recall what an earlier waking did — witness current state before you append, or you will record it twice.
